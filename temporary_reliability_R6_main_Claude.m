...
Obj = TAU_UP_SW  * ((full(p_feeder_d) - p_mat_d) * lam) ...             % ①上游节点：τ_UP_SW
    + TAU_TIE_SW * ((full(p_feeder_d) - p_mat_d) .* double(logical(round(Q_mat)))) * lam ...    % ②转供恢复：τ_TIE_SW
    + ((full(p_feeder_d) - p_mat_d) .* (1-double(logical(round(Q_mat))))) * (lam .* trp)...       % ③等待修复：τ_RP
    + (sum(sum(L_mat))/100) * (full(p_feeder_d) - p_mat_d) .* (double(logical(round(Q_mat))).*logical(L_mat)) * (lam .* trp) / sum(P_free);

sol = optimize(C, Obj, opts_milp);
...
total_shed_pu = sum(sum(L_res))/100;   % 所有场景总切负荷量（pu·场景）
...
CID = TAU_UP_SW  * (p_upstream_d * lam) ...             % ①上游节点：τ_UP_SW
    ...
    + total_shed_pu * p_direct_d .* (q_mat_d.*logical(L_res)) * (lam .* trp) / sum(P_free);
...
t_milp_elapsed = toc(t_milp);
fprintf('   MILP 完成（%.1f 秒），总切负荷=%.3f pu·场景\n', ...
    t_milp_elapsed, n_recovered, n_affected, total_shed_pu);
