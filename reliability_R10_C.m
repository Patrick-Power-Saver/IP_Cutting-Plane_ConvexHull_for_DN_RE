...
% ── 设备故障率 ────────────────────────────────────────────────
LAMBDA_TRF = 0;   % 变压器故障率（0=统一用线路公式，>0用固定值）
...
SHED_LIMIT_BASE  = [0.00, 0.25, 1.00];   % 基准切负荷上限比例
...
%    = 0.0 → L3无柔性（退化为普通可切负荷）
FLEX_RATIO_L3 = 0.8;   % 80%的L3负荷具备快速响应能力
...
USE_NODE_SPECIFIC_LEVELS = false;   % true会增加预处理时间但更真实
...
perm = randperm(nL);
idx_L1 = sort(perm(1:nL1));
idx_L2 = sort(perm(nL1+1:nL1+nL2));
idx_L3 = sort(perm(nL1+nL2+1:end));

% 个体化VOLL（每节点有独立扰动）
voll_vec = zeros(nL, 1);
voll_vec(idx_L1) = VOLL_BASE(1) * (1 + VOLL_NOISE_RATIO*(2*rand(nL1,1)-1));
voll_vec(idx_L2) = VOLL_BASE(2) * (1 + VOLL_NOISE_RATIO*(2*rand(nL2,1)-1));
voll_vec(idx_L3) = VOLL_BASE(3) * (1 + VOLL_NOISE_RATIO*(2*rand(nL3,1)-1));

% 切负荷上限（含L3柔性调整）
shed_limit_vec = zeros(nL, 1);
shed_limit_vec(idx_L1) = SHED_LIMIT_BASE(1);   % 0%，严禁切除
shed_limit_vec(idx_L2) = SHED_LIMIT_BASE(2);   % 25%，有限切除
shed_limit_vec(idx_L3) = SHED_LIMIT_BASE(3) * FLEX_RATIO_L3;   % L3×柔性率
...
fprintf('   负荷分级: L1(关键)=%d(%.0f%%), L2(重要)=%d(%.0f%%), L3(柔性)=%d(%.0f%%)\n', ...
...
fprintf('   W_cid: max=%.4f, mean=%.4f (CID最小化权重)\n', max(W_cid(:)), mean(W_cid(W_cid>0)));
fprintf('   W_shed VOLL差异化: L1平均%.0f, L2平均%.0f, L3平均%.0f\n', ...
    mean(W_shed(idx_L1,:),'all')*1e3, mean(W_shed(idx_L2,:),'all')*1e3, mean(W_shed(idx_L3,:),'all')*1e3);
...
fprintf('   稀疏ZZ优化: 可切节点=%d/%d, ZZ行数=%d (原版=%d, 压缩%.1f%%)\n', ...
    n_shed, nL, total_ZZ_rows, nL*N_LEVELS, (1-total_ZZ_rows/(nL*N_LEVELS+1e-9))*100);
