...
match_size_loc = probSettingSet.get_level_values(0) == (n,m) # (100, 50)
probSettingSet =  probSettingSet[match_size_loc]


probName, (data, option) = dataOptionDict_repeat[1][probSettingSet[1]] # v0_on=5
dataOptionDict, probSettingSet = varying_a0(data, option, a0_list=np.linspace(0,.99,linspaceNum))

os.makedirs('./output_assortment_map/dataset/', exist_ok=True)
filename = './output_assortment_map/dataset/dataOptionDict'+data.probType + f"_varying_a0_m{data.numProd}_n{data.numCust}"
