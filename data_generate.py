...
Note:
    From run_forTables_V1: generate data and options and save them
"""
...
#%%
repeatNum = int(sys.argv[1]) # repeatNum
...
#%%
numProd_numPust_list = [(100, 50), (150, 75), (200, 100)]
v0_off_v0_on_list  = [(1,2), (1, 5), (1, 10)]
arriveRatio_off_list = [0.5]
luce_on_list = [0, 1]
capacity_off_on_list = [(1, 1)]
...
option.variableNeed = ['Runtime', 'Separtime', 'addConstrTime', 'ObjVal','NumAssort','NumAssort_onAvg', 'gap', 'Status'] +rootnodeGap+ [ 'e_gap', 'NumConstrs', 'NodeCount', 'R_Status', 'R_Runtime', 'ObjRoot', 'ObjCtn', 'ObjBound', 'ObjValLuce']
...
for r in range(repeatNum):
    ...
    for s, probSetting_info in enumerate(probSettingSet):
        (numProd, numCust), arriveRatio_off, (v0_off, v0_on), luce, (kappa_off, kappa_on), (knapsack_off, knapsack_on) = probSetting_info
        
        print('\n\n==============================================\n')
        print('======{}th prob with setting:'.format(s)+str(probSetting_info))
        print('\n==============================================\n\n')

        if (s > 0) & ((knapsack_off, knapsack_on) == probSettingSet[s-1][5]) & ((numProd, numCust) == probSettingSet[s-1][0]):
...
filename = './DataSet/dataOptionDict'+data.probType + '_repeat%d_'%repeatNum + time_stamp_str
